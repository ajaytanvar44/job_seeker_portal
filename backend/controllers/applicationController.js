...
// ✅ Employer gets applications, Job Seeker gets empty list
export const employerGetAllApplications = catchAsyncError(async (req, res, next) => {
    ...
    // Employer case → fetch applications for employer
...
// ✅ Job Seeker gets applications, Employer gets empty list
export const jobSeekerGetAllApplications = catchAsyncError(async (req, res, next) => {
    ...
    // Job Seeker case → fetch applications for applicant
...
export const employerGetAllApplications = catchAsyncError(async (req, res, next) => {
    const { role } = req.user;
    if (role !== "Job Seeker") {
        return next(new ErrorHandler("Job Seeker is not allowed to access this resource", 403));
    }
    const { _id } = req.user;
    const applications = await Application.find({ 'employerID.user': _id });
    res.status(200).json({
