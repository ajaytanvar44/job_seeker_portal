...
export const postJob = catchAsyncError(async (req, res, next) => {
  ...
  if (fixedSalary && SalaryFrom && fixedSalary) {
    return next(new ErrorHandler("cannot enter fixed salary and range salary together!", 400));
  }
...
export const updateJob = catchAsyncError(async (req, res, next) => {
  ...
  if (!job) {
    return next(
      new ErrorHandler("Opps Job not found!", 404)
    );
  }

  job = await Job.findByIdAndUpdate(id, req.body, { new: true, runValidators: true ,userFindAndModify: false})
  res.status(200).json({
    success: true,
    job,
